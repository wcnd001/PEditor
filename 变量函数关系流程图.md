...
| 文件 | 变量/函数 | 影响 |
|---|---|---|
| `webcontrol.py` | `loop_marker_edit`、`loop_start_edit`、`loop_stop_edit` | 循环开始/结束步骤的循环标记和范围设置 |
...
## 22. 2026-05-30 浏览器导入字段同源渲染与 JS 安全转义（v3.8）

```mermaid
flowchart TB
    copy_click[复制按钮点击] --> resolve_field[resolve_copy_field_content]
    browser_export[导出至浏览器] --> build_fields[build_browser_rendered_fields]
    build_fields --> resolve_field
    resolve_field --> datamatcher[DataMatcher.render 单字段渲染]
    datamatcher --> rendered_fields[rendered_fields 字段缓存]
    rendered_fields --> web_payload[浏览器 payload]
    web_payload --> web_lookup[BrowserEngine._payload_lookup_static]
    web_lookup --> browser_step[浏览器步骤模板替换/表达式计算]

    js_step[执行JavaScript命令] --> js_render[render_javascript_template]
    js_render --> json_escape[json.dumps 自动 JS 字符串转义]
    json_escape --> safe_js[安全执行 JS]

    input_text[普通输入文本] --> ol_li[网页自动生成 ol/ul/li]
    ol_li --> clear_numbering[去除自动编号步骤]
    clear_numbering --> p_tags[转换为 p 段落]
```
...
| 名称 | 类型 | 作用 | 影响范围 |
|---|---|---|---|
| `resolve_copy_field_content()` | 主窗口函数 | 返回与复制按钮一致的单字段渲染结果 | 复制按钮、浏览器导入字段缓存 |
| `build_browser_rendered_fields()` | 主窗口函数 | 导出浏览器前生成所有字段的同源渲染缓存 | `export_current_to_browser` |
| `rendered_fields` | payload 字段 | 浏览器步骤优先读取的已渲染字段 | `BrowserEngine._payload_lookup_static` |
| `render_javascript_template()` | 浏览器引擎函数 | JS 专用 `[[字段名]]` 替换并自动安全转义 | 执行JavaScript命令 |
| `_clear_auto_numbering()` | 浏览器引擎函数 | 将 `ol/ul > li` 转换为普通 `p` | 去除自动编号动作 |

### 22.2 注意事项

1. 复制按钮现有界面、选中、拖动、剪贴板逻辑不改变，只把背后的稳定取值函数复用给浏览器导入。
2. 浏览器字段查找顺序调整为 `rendered_fields -> final_fields -> data_pool -> input_values`，避免优先拿到未渲染公式文本。
3. JS 命令中推荐写 `arguments[0].textContent = [[字段名]];`，不要手动给 `[[字段名]]` 加引号。
4. “去除自动编号”应通常放在普通输入文本之后、切换 iframe 之后使用，定位值常用 `//body`。


## 23. 2026-05-30 浏览器启动稳定参数补充（v3.8）

### 23.1 启动参数流程

```mermaid
flowchart TD
    launch_browser[BrowserEngine.launch_browser] --> build_options[_build_options]
    build_options --> remote_debug[--remote-debugging-port]
    build_options --> base_args[原有参数: disable-popup-blocking / start-maximized]
    build_options --> stable_args[新增保持活跃参数]
    stable_args --> timer[--disable-background-timer-throttling]
    stable_args --> renderer[--disable-renderer-backgrounding]
    stable_args --> occluded[--disable-backgrounding-occluded-windows]
    build_options --> create_driver[_create_driver]
```
...
| 文件 | 改动 | 影响范围 |
| --- | --- | --- |
| `webengine.py` | `launch_browser()` 的 `extra_args` 增加 3 个 Chrome 启动参数 | 仅影响由程序启动的受控 Chrome |
| `PEditor_教程.txt` | 补充稳定参数说明 | 让用户知道这些参数用于保持后台/被遮挡页面活跃 |
| `项目背景.txt` | 补充浏览器连接稳定性说明 | 项目背景与当前代码一致 |

### 23.3 稳定参数说明

- `--disable-background-timer-throttling`：减少后台页面 JavaScript 定时器被 Chrome 降频。
- `--disable-renderer-backgrounding`：减少后台标签页渲染进程被降级。
- `--disable-backgrounding-occluded-windows`：减少 Chrome 窗口被其它窗口遮挡后被后台化。

### 23.4 注意事项

1. 本次只增加启动参数，不加入自动重连、心跳检测或独立用户目录等其它连接恢复功能。
2. 这三个参数只能降低后台/遮挡导致的不稳定概率，不能替代 ChromeDriver 与 Chrome 主版本一致、网页登录状态有效等基础条件。

## 24. 2026-06-02 统计元素数量、循环变量起止、拖拽优化与测试范围控制（v3.9）

### 24.1 浏览器流程变量与函数关系

```mermaid
flowchart TB
    count_step[统计元素数量步骤] --> count_ui[webcontrol: count_result_name_edit]
    count_ui --> save_step[current_step_dict 保存 count_result_name]
    save_step --> flow_config[template_db.browser_flows]
    flow_config --> execute_flow[BrowserEngine.execute_flow]
    execute_flow --> count_elements[_count_elements]
    count_elements --> store_var[_store_payload_variable]
    store_var --> payload[payload 顶层变量 + data_pool]
    payload --> lookup[_payload_lookup_static]
    lookup --> loop_start[_resolve_loop_bound 解析循环起始]
    lookup --> loop_stop[_resolve_loop_bound 解析循环结束]
    loop_start --> loop_stack[loop_stack 当前循环上下文]
    loop_stop --> loop_continue[循环继续或结束]
```
...
```mermaid
flowchart TB
    drag_ui[webcontrol 拖拽速度/等待设置] --> drag_fields[drag_steps / drag_move_seconds / drag_hold_seconds / drag_hover_seconds / drag_wait_target_ready]
    drag_fields --> current_step[current_step_dict]
    current_step --> webengine_drag[BrowserEngine._drag_drop_element]
    webengine_drag --> find_source[定位源元素]
    webengine_drag --> find_target[定位目标元素]
    find_source --> hold[click_and_hold 后按住等待]
    hold --> move_path[按移动步数分段 move_by_offset]
    move_path --> wait_target[可选: 释放前重新识别目标]
    wait_target --> hover[目标区域悬停等待]
    hover --> release[释放鼠标]
```
...
```mermaid
flowchart TB
    test_ui[测试导入按钮旁 test_start_step_edit / test_end_step_edit] --> parse_limit[_parse_test_step_limit]
    parse_limit --> test_import[BrowserFlowWindow.test_import]
    test_import --> collect_flow[collect_flow]
    collect_flow --> execute_flow_range[BrowserEngine.execute_flow start_step_no + end_step_no]
    execute_flow_range --> start_index[_resolve_step_index 解析开始步骤]
    execute_flow_range --> end_index[_resolve_step_index 解析结束步骤]
    start_index --> run_range[从开始步骤索引执行]
    end_index --> stop_range[到结束步骤索引后停止]
    run_range --> stop_range
```
...
| 文件 | 名称 | 类型 | 作用 | 影响范围 |
|---|---|---|---|---|
| `webcontrol.py` | `ACTION_COUNT_ELEMENTS` | 动作常量 | 新增“统计元素数量”动作 | 步骤动作下拉框、流程保存 |
| `webcontrol.py` | `count_result_name_edit` | 输入控件 | 设置统计结果绑定变量名 | 当前步骤配置 |
| `webcontrol.py` | `loop_start_edit`、`loop_stop_edit` | 输入控件 | 循环起始/结束支持数字或变量 | 循环开始、循环结束 |
| `webcontrol.py` | `drag_steps_spin` 等拖拽控件 | 输入控件 | 控制拖拽步数、速度、等待 | 拖拽元素步骤 |
| `webcontrol.py` | `test_start_step_edit` / `test_end_step_edit` | 输入控件 | 控制测试导入执行的开始步骤和结束步骤；留空表示不限制 | 测试导入，不影响正式导出 |
| `webengine.py` | `_count_elements()` | 执行函数 | 统计定位匹配的元素数量 | 统计元素数量动作 |
| `webengine.py` | `_store_payload_variable()` | 执行函数 | 将统计结果写入 payload 顶层变量和 data_pool | 后续模板替换、循环起止解析 |
| `webengine.py` | `_resolve_loop_bound()` | 执行函数 | 将循环起始/结束的数字或变量解析为整数 | 循环开始、循环结束 |
| `webengine.py` | `_drag_drop_element()` | 执行函数 | 按人工拖拽路径和等待参数执行拖拽 | 拖拽元素动作 |
| `webengine.py` | `execute_flow(..., start_step_no=1, end_step_no=0)` | 执行入口 | 支持测试导入按开始/结束步骤限制执行范围 | 正式导出默认仍全流程执行 |

### 24.5 注意事项

1. 正式“导出至浏览器”不传测试范围，仍默认全流程执行；只有浏览器配置窗口的“测试导入”会读取开始步骤和结束步骤。
2. “统计元素数量”绑定变量后，后续步骤可直接在循环起始/结束中填写变量名，例如 `n`；也支持 `{n}` 或 `[[n]]` 写法。
3. 如果从循环块中间开始测试，前面的循环开始步骤不会执行，因此 `[[i]]` 等循环变量不会自动产生；调试循环时建议从对应的“循环开始”步骤开始。
4. 拖拽速度相关字段只影响“拖拽元素”动作，不改变点击、输入、自动填单元格等其它动作。
5. 本次没有擅自加入“第 i 个 table 是否包含准备并跳转”的新专用动作，该需求仍可后续单独分析页面条件判断是否需要增强。
