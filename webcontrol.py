...
class BrowserFlowWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.inspect_btn = QPushButton('自动获取页面元素')
        self.inspect_btn.clicked.connect(self.inspect_page_elements)
        ...
        elements_group = QGroupBox('页面元素自动获取结果（双击可填入当前步骤）')
        eg_layout = QVBoxLayout(elements_group)
        self.element_table = QTableWidget()
        self.element_table.setColumnCount(8)
        self.element_table.setHorizontalHeaderLabels(['标签', '文本', 'id', 'name', 'placeholder', 'title', 'css', 'xpath'])
        self.element_table.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
        ...
        self.element_table.itemDoubleClicked.connect(self.apply_selected_element_to_step)
        eg_layout.addWidget(self.element_table, 1)
        element_btns = QHBoxLayout()
        self.apply_element_btn = QPushButton('将选中元素填入当前步骤')
        self.apply_element_btn.clicked.connect(self.apply_selected_element_to_step)
        element_btns.addWidget(self.apply_element_btn)
        self.add_click_step_btn = QPushButton('为选中元素新增点击步骤')
        ...
        self.add_input_step_btn = QPushButton('为选中元素新增输入步骤')
    ...
    def update_action_visibility(self, action):
        locator_needed = action in (ACTION_CLICK, ACTION_INPUT, ACTION_WAIT_ELEMENT, ACTION_SWITCH_IFRAME)
    ...
    def inspect_page_elements(self):
        """Automatically fetch elements from the current page.

        If the browser is not connected, this method will not attempt to
        launch a browser.  It will instead inform the user to start or
        connect a browser first.  This prevents unintentionally opening
        a new browser window when the user clicks the button.
        """
        # Do not auto-launch a browser when inspecting elements.
        if not getattr(self.engine, 'is_connected', lambda: False)():
            QMessageBox.information(self, '提示', '浏览器未连接，请先启动或连接浏览器后再获取页面元素。')
            self.log('浏览器未连接，无法获取页面元素。')
            return
        try:
            elements = self.engine.inspect_elements()
            ...
            for row, item in enumerate(elements):
                ...
                for col, value in enumerate(values):
                    self.element_table.setItem(row, col, QTableWidgetItem(str(value)))
            self.log(f'已自动获取页面元素：{len(elements)} 个。')
        except Exception as e:
            QMessageBox.critical(self, '错误', f'获取页面元素失败：{e}')
            self.log(f'获取页面元素失败：{e}')

    def selected_element_info(self):
        row = self.element_table.currentRow()
        if row < 0:
            return None
        keys = ['tag', 'text', 'id', 'name', 'placeholder', 'title', 'css', 'xpath']
        data = {}
        for col, key in enumerate(keys):
            item = self.element_table.item(row, col)
    ...
    def choose_best_locator(element):
        ...
        return 'xpath', element.get('xpath', '')

    def apply_selected_element_to_step(self, *args, **kwargs):
        element = self.selected_element_info()
        if not element:
            QMessageBox.information(self, '提示', '请先选择一个自动获取到的页面元素。')
            return
        locator_type, locator_value = self.choose_best_locator(element)
        action = self.action_combo.currentText()
        if action not in (ACTION_CLICK, ACTION_INPUT, ACTION_WAIT_ELEMENT, ACTION_SWITCH_IFRAME):
        ...
        self.log(f'已将元素定位填入当前步骤：{locator_type} = {locator_value}')
